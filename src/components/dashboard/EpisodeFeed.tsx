      ...
      <div className="space-y-4">
        {episodes?.length === 0 ? (
          <div className="text-center py-20 bg-zinc-900/30 border border-zinc-800 rounded-3xl">
             ...
             <p className="text-zinc-600 mt-2">Trigger an update from one of your sources to begin.</p>
          </div>
        ) : (
          episodes?.map((episode) => (
            <div key={episode.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:border-zinc-700 transition-colors">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center border ${
                  episode.status === 'done' ? 'bg-green-500/10 border-green-500/20 text-green-500' :
                  episode.status === 'failed' ? 'bg-red-500/10 border-red-500/20 text-red-500' :
                  'bg-indigo-500/10 border-indigo-500/20 text-indigo-500'
                }`}>
                  {episode.status === 'done' ? <CheckCircle2 className="w-6 h-6" /> :
                   episode.status === 'failed' ? <AlertCircle className="w-6 h-6" /> :
                   <RefreshCw className="w-6 h-6 animate-spin" />}
                </div>
                <div>
                  <h3 className="font-bold text-zinc-100 text-lg">{episode.title}</h3>
                  <div className="flex items-center gap-2 text-sm text-zinc-500">
                    <span className="px-2 py-0.5 bg-zinc-800 rounded text-zinc-400 text-xs font-mono uppercase">{episode.sourceName}</span>
                    <span>•</span>
                    <span>{episode.createdAt ? new Date(episode.createdAt).toLocaleString() : 'Just now'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                {episode.status === 'pending' && (
                   <span className="text-zinc-500 text-sm italic animate-pulse">Synthesizing...</span>
                )}
              </div>
            </div>
          ))
